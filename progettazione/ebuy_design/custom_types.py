...
class Matricola(int):
    ...
    def __new__(cls, m: int) -> Self:

        m = super().__new__(cls, m)
        ...
        raise ValueError(f"la stringa {m} non è un codice fiscale valido")


class RealZO(float):

    def __new__(cls, r: int) -> Self:

        r = super().__new__(cls, r)

        if 0 <= r <= 1:

            return r
        
        raise ValueError("il reale non è compreso tra zero e uno")
...
class RealGZ(float):
    ...
    def __new__(cls, r: int) -> Self:
        ...
        raise ValueError("il reale non è minore di 0")
